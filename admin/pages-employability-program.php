                  ...
                  <table id="dataTableBasic" class="table table-hover table-sm display no-wrap shadow" style="width:100%">
                    <thead class="bg-gradient bg-info text-white">
                      <tr class="text-center">
                        ...
                        <th scope="col" class="border-0" width="430px">Employability Program</th>
                    ...
                    <tbody class="align-middle">
                      ...
                      $querycourse = $conn->query("SELECT * FROM employability_program
                                               ...
                                               WHERE course_created_by = '$get_userID' AND course_deleted_date IS NULL;");

                      $num = 1;
                      if (mysqli_num_rows($querycourse) > 0) {
                        while ($rows = mysqli_fetch_object($querycourse)) {
                          $course_id = $rows->ep_id;
                          $ep_id = $rows->ep_id;
                      ?>
                          <tr>
                            ...
                            <td class="border-top-0">
                              <a class="text-inherit">
                                <div class="d-lg-flex align-items-center">
                                  <div>
                                    <img src="../assets/images/employability_program/epthumbnails/<?php echo $rows->ep_cover_attachment; ?>" alt="" class="img-4by3-lg rounded" />
                                  </div>
                            ...
                            <?php if ($rows->ep_fee == 0 || $rows->ep_fee == 0 || $rows->ep_fee == 0) { ?>
                              <td class="text-center">Free<?php #echo $rows->ep_fee;
                                                          ?></td>
                            <?php } else { ?>
                            ...
                            <td class="text-center">
                              <span style="vertical-align: middle;" class="<?php if ($rows->ep_publish == 'Draft') {
                                                                              echo "badge rounded-pill bg-secondary";
                                                                            } elseif ($rows->ep_publish == 'Processing') {
                                                                              echo "badge rounded-pill bg-warning";
                                                                            } elseif ($rows->ep_publish == 'Approved') {
                                                                              echo "badge rounded-pill bg-success";
                                                                            } elseif ($rows->ep_publish == 'Published') {
                                                                              echo "badge rounded-pill bg-primary";
                                                                            } else {
                                                                              echo "badge rounded-pill bg-danger";
                                                                            } ?>"><?php echo $rows->ep_publish; ?></span>
                            </td>
                            <?php if ($rows->ep_total_enrolled != NULL) { ?>
                              <td class="text-center"><?php echo $rows->ep_total_enrolled; ?></td>
                            <?php } else { ?>
...
WHERE employability_program.ep_id = '$ep_id' AND employability_program.course_created_by = '$get_userID' AND employability_program_quiz.epq_created_by ='$get_userID'");
                              ...
                              <?php if ($rows->ep_publish == 'Draft') {
                                ...
                                if ($totallearningvideo >= 1 && $totallearningactivity >= 1 && $totalassessment >= 1) {
                                  // if ( $totallearningvideo >= 1 && $totallearningactivity >= 1 ) {
                              ?>

                                  <a class="btn btn-sm btn-success shadow" href="admin-function.php?publish_course=<?php echo $rows->ep_id; ?>" title="Publish Course" onclick="return publishcourse()">Publish</a>
                                <?php } else { ?>
                                ...
                                <?php } ?>
                              <?php } else { ?>
                                <a class="btn btn-sm btn-warning shadow" href="admin-function.php?unpublish_course=<?php echo $rows->ep_id; ?>" title="Unpublish Course" onclick="return unpublishcourse()">Unpublish</a>
                              <?php  } ?>
                            ...
                            <td class="text-muted px-4 py-3 align-middle border-top-0">
                              <span class="dropdown dropstart">
                                ...
                                <span class="dropdown-menu" aria-labelledby="courseDropdown"><span class="dropdown-header">Action</span>
                                  ...
                                  <a class="dropdown-item" href="pages-employability-program-edit.php?cid=<?php echo $rows->ep_id; ?>"><i class="fe fe-edit dropdown-item-icon text-warning"></i>Edit</a>
                                  <a class="dropdown-item" href="pages-employability-program-progress-history.php?cid=<?php echo $rows->ep_id; ?>"><i class="fe fe-users dropdown-item-icon text-warning"></i>View Enrolled Students</a>
                                  <a class="dropdown-item" href="admin-function.php?delete_ep=<?php echo $rows->ep_id; ?>" title="Delete Course" onclick="return deletecourse()"><i class="fe fe-trash dropdown-item-icon text-danger"></i>Delete</a>
                                </span>
                              </span>
                            </td>
                          </tr>
