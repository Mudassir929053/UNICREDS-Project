...
if (isset($_POST['edit_psychometric_quiz_question'])) {
   ...
   } elseif(@$newqueimg == NULL && $new_quiz_question != NUll){
...
if (isset($_POST['add_job_advertisement'])) {
   ...
   $folder1 = "../assets/attachment/advertisement_Attachment/";
...
/* ------------End of add job Advertisement----------*/

/* ------------edit job Advertisement----------*/
if (isset($_POST['edit_job_advertisement'])) {
   // Get the current industry ID from the session
   $industry_id = $_SESSION['sess_industryid'];

   // Get the ID of the advertisement to be edited
   $pa_id = $_POST['pa_id'];
   ...
   $prev_attachment = $_POST['Prev_attachment_name'];

// Check if a new file is uploaded
if ($_FILES['pa_attachment']['name'] != "") {
    // Generate a new filename for the uploaded file
    $new_pa_attachment = str_replace("'", "", date('YmdHis') . $_FILES['pa_attachment']['name']);
    // Delete the previous file if it exists
    if ($prev_attachment != "" && file_exists("../assets/attachment/advertisement_Attachment/" . $prev_attachment)) {
        unlink("../assets/attachment/advertisement_Attachment/" . $prev_attachment);
    }
    // Upload the new file
    $folder1 = "../assets/attachment/advertisement_Attachment/";
    ...
    $new_pa_attachment = $prev_attachment;
}

   // Get the current date and time for the updated_on field
   $pa_updated_on = date('Y-m-d H:i:s');

   // Update the advertisement record in the database
   $query = "UPDATE project_advertisement SET pa_advrt_title = '$new_pa_advrt_title', pa_start_date = '$formatted_start_date', pa_duration = '$new_pa_duration', pa_advrt_des = '$new_pa_advrt_des', pa_advrt_type = '$new_pa_advrt_type', pa_category = '$new_pa_category', pa_salary = '$new_pa_salary', pa_salary_min = '$new_pa_salary_min', pa_salary_max = '$new_pa_salary_max', pa_vacancy = '$new_pa_vacancy', pa_requirement = '$new_pa_requirement', pa_updated_on = '$pa_updated_on'";

   // Check if a new file is uploaded to update pa_attachment
   if ($_FILES['pa_attachment']['name'] != "") {
       $query .= ", pa_attachment = '$new_pa_attachment'";
   }
   ...
   // Execute the query
   $editJob = $conn->query($query);

   // Redirect back to the previous page if the update is successful, or show an error message if it fails
   if ($editJob) {
       echo ("<script>location.href='$_SERVER[HTTP_REFERER]';</script>");
   } else {
...
if (isset($_POST['assign_test'])) {
   ...
   $assign_test = $conn->query("INSERT INTO assign_test (at_su_id, at_job_id, at_ltq_id, at_st_id, at_pt_id,at_assigned_by,at_created_date,at_expiry_date) VALUES ('$su_id', '$job_id', '$language_test', '$skill_assessment_test', '$psychometric_test','$get_userID',NOW(),DATE_ADD(NOW(), INTERVAL 1 DAY))");
   ...
   if ($assign_test) {
      ...
      // Deadline is set to 1 day from now 
      ...
      if ($insert_notification) {
         ...
         echo "<script>alert('Assign is not successful'); location.href = '$_SERVER[HTTP_REFERER]';</script>";
      }
   } else {
      echo "<script>alert('Assign is not successful'); location.href = '$_SERVER[HTTP_REFERER]';</script>";
   }
