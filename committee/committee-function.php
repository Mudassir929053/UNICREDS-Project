...
if (isset($_POST['update_ProfilePic'])) {
    ...
    $profile_picture = time() . '_' . $_FILES['committee_logo']['name'];

    $target = '../assets/images/avatar/' . $profile_picture;

    if (move_uploaded_file($_FILES['committee_logo']['tmp_name'], $target)) {
        $updatePP = $conn->query("UPDATE committee 
                                SET committee_logo = '$profile_picture'
        ...
        echo "<script>alert('Failed to upload new picture');
					location.href = '$_SERVER[HTTP_REFERER]';</script>";
    }
}
...
if (isset($_POST['edit_profile'])) {
    ...
    $editProfile = $conn->query("UPDATE committee SET committee_email = '$committee_email', committee_contact_no = '$committee_contact_no', committee_address = '$committee_address' 
                                 WHERE committee_id = '$committee_id' AND committee_user_id = '$committee_user_id'");

    if ($editProfile) {
        $editUser = $conn->query("UPDATE user SET user_username = '$committee_email' WHERE user_id = '$committee_user_id'");
...
if (isset($_POST['add_announcement']))
{
	...
	$addAnnouncement = $conn -> query ("INSERT INTO announcement_committee (announcement_title, announcement_receiver, announcement_message, announcement_attachment, announcement_created_by, announcement_created_date)
										VALUES ('$announcement_title', '".$checkbox1."', '$announcement_message', '$announcement_attachment', '$committee_id', '$announcement_date_created')");
...
if (isset($_POST['edit_announcement']))
{
	...
	if($newfilename != NULL){
		$checkAnnouncementFile = $conn -> query("SELECT announcement_attachment FROM announcement_committee WHERE announcement_id = '$announcement_id'");
		...
		if($checkAnnouncementRow -> announcement_attachment != NULL){
			...
			$editAnnouncement = $conn -> query("UPDATE announcement_committee SET announcement_title = '$new_title', announcement_receiver = '$new_receiver', announcement_message = '$new_message', announcement_attachment = '$newfilename'
		...
		$editAnnouncement = $conn -> query("UPDATE announcement_committee SET announcement_title = '$new_title', announcement_receiver = '$new_receiver', announcement_message = '$new_message'
											WHERE announcement_id = '$announcement_id'");
...
if(isset($_GET['delete_announcement'])){
	...
	$deleteAnnouncement = $conn -> query ("DELETE FROM announcement_committee where announcement_id = '$delete_announcement'");
...
if (isset($_POST['edit_mc'])) {
	...
	if ($newfileimg != NULL) {
		...
		if ($checkimgRow->mc_image != NULL) {
			...
			$editmc = $conn->query("UPDATE microcredential SET mc_title = '$new_mc_name', mc_code = '$new_mc_code', mc_description = '$new_mc_desc', mc_category = '$new_mc_category', mc_level = '" . $new_mc_level . "', mc_duration = '$new_mc_duration', mc_fee = '$new_mc_fee', mc_image = '$newfileimg' WHERE mc_id = '$mc_id'");
		...
		$editmc = $conn->query("UPDATE microcredential SET mc_title = '$new_mc_name', mc_code = '$new_mc_code', mc_description = '$new_mc_desc', mc_category = '$new_mc_category', mc_level = '" . $new_mc_level . "', mc_duration = '$new_mc_duration', mc_fee = '$new_mc_fee' WHERE mc_id = '$mc_id'");
...
/* ------------approve micro-credential----------*/
if (isset($_POST['approve_mc'])) {

	$mc_id	= $_POST['mc_id'];
	$rmc_id	= $_POST['rmc_id'];
	$rmc_user_review = $_POST['rmc_user_review'];
	$rmc_user_request = $_POST['rmc_user_request'];
	$rmc_comment = mysqli_real_escape_string($conn, $_POST['rmc_comment']);
	$mc_status = "Approved";
	$mc_approved_date = date('Y-m-d H:i:s');

	$reviewmc = $conn->query("UPDATE review_microcredential SET rmc_user_review = '$rmc_user_review', rmc_status = '$mc_status', rmc_comment = '$rmc_comment', rmc_date_review = '$mc_approved_date' 
							  WHERE rmc_id  = '$rmc_id' AND rmc_mc_id = '$mc_id'");

	if ($reviewmc) {

		$approvemc = $conn->query("UPDATE microcredential SET mc_status = '$mc_status' WHERE mc_id = '$mc_id'");

		if ($approvemc) {
		echo ("<script>window.location.href ='pages-microcredential-review.php'</script>");
		$_SESSION['tab_type'] = $mc_status;
		} 
		else {
			echo "<script>alert('Approve micro-credential is not successful.');
			location.href = '$_SERVER[HTTP_REFERER]';</script>";
		}
	}

	 else {
		echo "<script>alert('Review micro-credential is not successful.');
		location.href = '$_SERVER[HTTP_REFERER]';</script>";
	}
}
/* ------------approve micro-credential----------*/

/* ------------reject micro-credential----------*/
if (isset($_POST['reject_mc'])) {
	...
	$rmc_comment = mysqli_real_escape_string($conn, $_POST['rmc_comment']);
	$mc_status = "Rejected";
	$mc_rejected_date = date('Y-m-d H:i:s');

	$reviewmc = $conn->query("UPDATE review_microcredential SET rmc_user_review = '$rmc_user_review', rmc_status = '$mc_status', rmc_comment = '$rmc_comment', rmc_date_review = '$mc_rejected_date'
							  WHERE rmc_id  = '$rmc_id' AND rmc_mc_id = '$mc_id'");

	if ($reviewmc) {

		$rejectmc = $conn->query("UPDATE microcredential SET mc_status = '$mc_status' WHERE mc_id = '$mc_id'");
		...
		$_SESSION['tab_type'] = $mc_status;
			...
			echo "<script>alert('Reject micro-credential is not successful.');
			location.href = '$_SERVER[HTTP_REFERER]';</script>";
		}
	}
...
if (isset($_POST['edit_mcq'])) {
	...
	$updatemcquiz = $conn->query("UPDATE mc_quiz SET mcq_title = '$new_mcq_title', mcq_instruction = '$new_mcq_instruction', mcq_duration = '$new_mcq_duration' 
