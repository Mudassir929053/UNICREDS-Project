...
<?php
    ...
    if(isset($_GET["course_id"])) {
        ...
        $quizAttempt = $courseInfo->fetch_quiz_result($quizID);

        // Check if the student already attempted the quiz or not.
        if($quizAttempt === NULL) {
            // --- fetch course based on subjID.
            $courseData = $courseInfo->fetch_course($subjID);
            ...
            $subjTitle = $courseData["course_title"];
            $quizTitle = $fetchQuizInfo["cq_title"];
            $quizTotalQuestion = $fetchQuizQnA !== NULL ? count($fetchQuizQnA) : 0;
            $quizInstruction = $fetchQuizInfo["cq_instruction"];
        } else {
            header("Location: course-learning-material.php?course_id=$subjID&pill=quiz");
        }
    } else if(isset($_GET["mc_id"])) {
        $subj_type="mc";
        $subjID = $_GET["mc_id"];
        $quizAttempt = $mcInfo->fetch_quiz_result($quizID);
        ...
        if($quizAttempt === NULL) {
            ...
            $quizTitle = $fetchQuizInfo["mcq_title"];
            $quizTotalQuestion = $fetchQuizQnA !== NULL ? count($fetchQuizQnA) : 0;
            $quizInstruction = $fetchQuizInfo["mcq_instruction"];
        } else {
