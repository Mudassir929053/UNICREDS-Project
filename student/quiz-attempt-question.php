...
<?php
    ...
    if(isset($_GET["course_id"])) {
        ...
        // --- fetch the course's quiz informations and questions and its answers choices.
        $fetchQuizInfo = $courseInfo->fetch_quiz_info($quizID);
        $fetchQuizQnA = $courseInfo->fetch_quiz_QnA($quizID);

        // Check if there's a questions or not.
        if($fetchQuizQnA !== NULL) {
            // --- fetch the necessary course's informations.
            $quizDuration = $fetchQuizInfo["cq_duration"];
            $quizExitLink = "course-learning-material.php?course_id=$subjID&pill=quiz";
            ...
            foreach($fetchQuizQnA as $val) {
                $data["qq_id"] = $val["cqq_id"];
                $data["qa_id"] = $val["cqa_id"];
                $data["qq_type"] = $val["cqq_type"];
                $data["qq_figure"] = $val["cqq_figure"] !== NULL ? "../assets/attachment/course/coursequiz/".$val["cqq_figure"] : $val["cqq_figure"];
                $data["qq_figcaption"] = $val["cqq_figure_caption"];
                $data["qq_question"] = $val["cqq_question"];
                $data["qa_1"] = $val["cqa_answer1"];
                $data["qa_2"] = $val["cqa_answer2"];
                $data["qa_3"] = $val["cqa_answer3"];
                $data["qa_4"] = $val["cqa_answer4"];
            ...
            header("Location: quiz-attempt-main.php?course_id=$subjID&quiz_id=$quizID");
        }
        ...
        $fetchQuizInfo = $mcInfo->fetch_quiz_info($quizID);
        $fetchQuizQnA = $mcInfo->fetch_quiz_QnA($quizID);

        // Check if there's a questions or not.
        if($fetchQuizQnA !== NULL) {
            // --- fetch the necessary micro-credential's informations.
            $quizDuration = $fetchQuizInfo["mcq_duration"];
            $quizExitLink = "micro-creds-learning-material.php?mc_id=$subjID&pill=quiz";
            ...
            $i = 0;
            foreach($fetchQuizQnA as $val) {
                $data["qq_id"] = $val["mcqq_id"];
                $data["qa_id"] = $val["mcqa_id"];
                $data["qq_type"] = $val["mcqq_type"];
                $data["qq_figure"] = $val["mcqq_figure"] !== NULL ? "../assets/attachment/microcredential/mcquiz/".$val["mcqq_figure"] : $val["mcqq_figure"];
                $data["qq_figcaption"] = $val["mcqq_figure_caption"];
                $data["qq_question"] = $val["mcqq_question"];
                $data["qa_1"] = $val["mcqa_answer1"];
                $data["qa_2"] = $val["mcqa_answer2"];
                $data["qa_3"] = $val["mcqa_answer3"];
                $data["qa_4"] = $val["mcqa_answer4"];
            ...
            header("Location: quiz-attempt-main.php?mc_id=$subjID&quiz_id=$quizID");
        }
    }
...
<body>
    ...
    <form id="qnaForm" action="function/quiz-test-attempt.php" method="post" enctype="multipart/form-data">
        ...
        <div class="py-6 mt-8">
            ...
            <?php
                    } else if($data["qq_type"] === "True/False") {
            ?>
                        <!-- Question Type 5 -->
                        <!-- *** True/False question -->
                        <div class="card mb-3 <?= $collapse ?>" id="question<?= $i + 1 ?>">
