        ...
        FROM (
            SELECT
                CASE
                    ...
                    WHEN r.role_name = 'Committee' THEN c.committee_user_id
                    WHEN r.role_name = 'Expert' THEN e.expert_user_id
                    WHEN r.role_name = 'Lecturer' THEN l.lecturer_user_id
                END AS id,
                CASE
                    WHEN r.role_name = 'Administrator' THEN CONCAT(a.admin_name)
                    WHEN r.role_name = 'Committee' THEN CONCAT(c.committee_name)
                    WHEN r.role_name = 'Expert' THEN CONCAT(e.expert_fname, ' ', e.expert_lname)
                    WHEN r.role_name = 'Lecturer' THEN CONCAT(l.lecturer_fname, ' ', l.lecturer_lname)
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                <div class="d-block">
                    <div class="form-check mb-1 ms-4 me-2">
                        ...
                        <label class="form-check-label" for="inst-<?= $c_id ?>"><?= $courseOwnerName ?></label>
                    </div>
                </div>
                <?php
            }
