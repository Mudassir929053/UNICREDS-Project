...
if (isset($_GET["quiz_id"])) {
    $subj_type = "lt";
    $subjID = $_GET["quiz_id"];
    $quizAttempt = $ltInfo->fetch_quiz_result($quizID);

    // Check if the student already attempted the quiz or not.
    if ($quizAttempt === null) {
        // --- fetch course based on subjID.
        $epData = $ltInfo->fetch_employability_programs($subjID);
        ...
        $quizTitle = $fetchQuizInfo["ltq_title"];
        $quizTotalQuestion = $fetchQuizQnA !== null ? count($fetchQuizQnA) : 0;
        $quizInstruction = $fetchQuizInfo["ltq_instruction"];
    } else {
        header(
            ...
            <div class="container-fluid px-0">
                ...
                <ul class="navbar-nav ms-auto flex-row align-items-center">
                    <li class="nav-item docs-header-btn">
                        ...
                        <button class="btn btn-danger btn-sm px-5 me-2 me-lg-5" type="button" onclick="window.open('<?= $quizExitLink ?>', '_self');">
                            Exit
                        </button>
                    </li>
