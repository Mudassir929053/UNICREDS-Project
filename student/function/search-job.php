    ...
    return (
        ...
        '<div class="px-6 mb-3">' . 
            ...
            '<div class="d-flex justify-content-between align-items-center mb-6">' . 
                ...
                '</div>' . 
                '<div>' . 
                    '<a href="job-apply.php?job_id='.$job_desc["job_id"].'" class="btn btn-primary">Apply Now</a>' . 
...
if(isset($_POST["fetchJobList"])) {
    ...
    if($sql_stmt === "") {
        // fetch all Job List.
        $jobList = $jobInfo->fetch_jobs($jsua_sql, $sql_limit);

        echo process_job_offers($jobList, $jobInfo->fetch_jobs($jsua_sql));
    } else {
        // fetch Job List based on $sql_stmt.
        $sql_opr = $jsua_sql.(($jsua_sql === "" || $sql_stmt === "") ? "" : " AND ").$sql_stmt;
