 ...
 * @param object $ep instantiation of __EmployabilityProgram__ class.
 * @return array an associative array.
 */
function process_data($type, $list, $c, $mc,$ep) {
    if($type === "c") {
        ...
        $stud_enr = $ep->fetch_enrolled_employability_programs();

        $id = $list["ep_id"];
        $link = "ep-enroll.php?ep_id={$id}";
        $enrolled = false;
        $image = "../assets/images/employability_program/epthumbnails/" . $list["ep_cover_attachment"];
        $title = $list["ep_title"];
        // $owner = $list["course_created_by"] !== NULL ? $list["institution_address"] : "<span class='text-muted'><em>Not available</em></span>";
        $category = $list["ep_category"] !== NULL ? $list["ep_category"] : "<span class='text-muted'><em>Not set</em></span>";
        // $duration = $list["ep_duration"];
        // $level = $list["ep_level"] !== NULL ? acadLevel($list["ep_level"]) : "<span class='text-muted'><em>Not set</em></span>";
        // $credit = $list["ep_credit_transfer"] !== NULL ? $list["ep_credit_transfer"] . " credits" : "<span class='text-muted'><em>Not set</em></span>";
        $total_enrolled = ($list["ep_total_enrolled"] == 0 || $list["ep_total_enrolled"] === NULL) ? "<span class='text-muted'><em>No enrollment</em></span>" : $list["ep_total_enrolled"] . " enrolled";
    }
    ...
    if($stud_enr !== NULL) {
        foreach($stud_enr as $val) {
            if($type === "c") {
                ...
                if($val["eepsu_ep_id"] === $id) {
                    $link = "ep-view-enrolled.php?ep_id={$id}&amp;pill=desc";
                    $enrolled = true;
                }
            }  
        }
 ...
 */
function fetch_list_info($type, $sql_opr, $sql_limit, $c, $mc, $ep) {
    $lists = array();
    ...
    if ($type === "c") {
        ...
        if ($sql_opr === "") {
            $data_list = $ep->fetch_employability_programs($sql_opr, $sql_limit);
            $total = $ep->fetch_employability_programs() !== NULL ? count($ep->fetch_employability_programs()) : 0;
        } else {
            $data_list = $ep->fetch_employability_programs($sql_opr, $sql_limit);
            $total = $ep->fetch_employability_programs($sql_opr) !== NULL ? count($ep->fetch_employability_programs($sql_opr)) : 0;
        }
    }

    if ($data_list !== NULL) {
        foreach ($data_list as $val) {
            array_push($lists, process_data($type, $val, $c, $mc, $ep));
        }

        // Sort the array by the "title" column
        $titles = array_column($lists, 'title');
        array_multisort($titles, SORT_ASC, SORT_STRING, $lists);

        if (count($lists) > 0) {
            return json_encode(
                array(
                    "grid"  => $type == 'ep' ? displayGrid1($lists) : displayGrid($lists),
                    "tab"   => $type == 'ep' ? displayTab1($lists) : displayTab($lists),
                    "count" => count($lists),
                    "total" => $total
                ),
...
//     if ($fetch_type === "all") {
//         echo fetch_list_info($type, "", $sql_limit, $courseInfo, $mcInfo, $epInfo);
...
//         } else if ($type === "ep") {
//             $inst_sql = $inst_ids_arr !== NULL ? "ep.course_created_by IN (" . parse_sql_IN($inst_ids_arr) . ")" : "";
//             $acad_sql = "";
//         } else {
//             $acad_sql = "";
//         }
...
//         echo fetch_list_info($type, $sql_opr, $sql_limit, $courseInfo, $mcInfo, $epInfo);
//     }
// }
...
if (isset($_POST["fetchList"])) {
    ...
    if ($fetch_type === "all") {
        ...
        if ($type === "c") {
            ...
            $inst_sql = $inst_ids_arr !== NULL ? "ep.ep_category IN (" . parse_sql_IN($inst_ids_arr) . ")" : "";
        } else {
            $acad_sql = "";
        }
