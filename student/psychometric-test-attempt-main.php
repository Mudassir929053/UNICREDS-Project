...
<?php
    ...
    if(isset($_GET["pt_id"])) {
        $subj_type="pt";
        $subjID = $_GET["pt_id"];
        $quizAttempt = $ptInfo->fetch_ltquiz_result($pt_id);

        // Check if the student already attempted the quiz or not.
        if($quizAttempt === NULL) {
            // --- fetch micro-credential based on subjID.
            $ptData = $ptInfo->fetch_ltquiz($subjID);
            // --- fetch the micro-credential's quiz informations and questions and its answers choices.
            $fetchQuizInfo = $ptInfo->fetch_ltquiz_info($pt_id);
            $fetchQuizQnA = $ptInfo->fetch_quiz_QnA($pt_id);

            // --- fetch the necessary micro-credential's informations.
            $quizDuration = $fetchQuizInfo["pt_duration"];
            $quizExitLink = "student-psychometric-test.php?pt_id=$subjID";
           
            $quizTitle = $fetchQuizInfo["pt_title"];
            $quizTotalQuestion = $fetchQuizQnA !== NULL ? count($fetchQuizQnA) : 0;
            $quizInstruction = $fetchQuizInfo["pt_instruction"];
        } else {
            header("Location: test.php?pt_id=$subjID");
        }
    }
                                    ...
                                    <form action="function/quiz-test-attempt.php" method="post" enctype="multipart/form-data">
                                        <div class="d-flex justify-content-center">
                                            <div class="d-grid col-6 mx-25">
                                                ...
                                                <input type="hidden" name="quiz_id" value="<?= $pt_id?>">
